OC.L10N.register(
    "viewer",
    {
    "Viewer" : "Visionneuse",
    "Simple file viewer with slideshow for media" : "Visionneuse simple de fichiers avec diaporama pour les médias",
    "Show your latest holiday photos and videos like in the movies, show a glimpse of your latest novel directly from your Nextcloud, choose the best GIF of your collection thanks to the direct preview of your favorites files and many more!" : "Montrez vos dernières photos et vidéos de vacances comme au cinéma, montrez un aperçu de votre dernier roman directement depuis votre Nextcloud, choisissez le meilleur GIF de votre collection grâce à l'aperçu direct de vos fichiers favoris et bien d'autres choses encore !",
    "Your browser does not support audio." : "Votre navigateur web ne prend pas en charge l'audio.",
    "Error loading {name}" : "Erreur lors du chargement de {name}",
    "You are about to overwrite the original file. Are you sure you want to continue?" : "Vous êtes sur le point de remplacer le fichier d'origine. Voulez-vous vraiment continuer ?",
    "A file with this name already exists. Do you want to overwrite it?" : "Un fichier portant ce nom existe déjà. Voulez-vous le remplacer ?",
    "Confirm overwrite" : "Confirmer le remplacement",
    "Overwrite" : "Remplacer",
    "Cancel" : "Annuler",
    "An error occurred while trying to confirm the file overwrite." : "Une erreur est survenue lors de la tentative de confirmation du remplacement du fichier.",
    "Image saved" : "Image enregistrée",
    "Error saving image" : "Erreur lors de l'enregistrement de l'image",
    "Play the live photo" : "Lire la Live Photo",
    "LIVE" : "LIVE",
    "Your browser does not support videos." : "Votre navigateur ne prend pas en charge les vidéos.",
    "This file no longer exists" : "Ce fichier n'existe plus",
    "There is no plugin available to display this file type" : "Il n'y a pas d'extension disponible pour afficher ce type de fichier.",
    "Failed to save file before download" : "Impossible d'enregistrer le fichier avant le téléchargement",
    "Edit" : "Modifier",
    "Exit full screen" : "Quitter le plein écran",
    "Full screen" : "Plein écran",
    "Open sidebar" : "Ouvrir le panneau latéral",
    "Download" : "Télécharger",
    "Delete" : "Supprimer",
    "View" : "Afficher",
    "Name" : "Nom",
    "Save" : "Enregistrer",
    "Save as" : "Enregistrer sous",
    "Back" : "Retour",
    "Loading …" : "Chargement …",
    "Reset" : "Réinitialiser",
    "All changes will be lost." : "Tous les changements seront perdus.",
    "Are you sure you want to continue?" : "Voulez-vous vraiment continuer ?",
    "Apply" : "Appliquer",
    "Warning" : "Avertissement",
    "Confirm" : "Confirmer",
    "Discard changes" : "Abandonner les modifications",
    "Undo" : "Annuler",
    "Redo" : "Rétablir",
    "Show original image" : "Afficher l'image originale",
    "Zoom in" : "Zoomer",
    "Zoom out" : "Dézoomer",
    "Toggle zoom menu" : "Afficher/Masquer le menu de zoom",
    "Adjust" : "Ajuster",
    "Fine-tune" : "Réglages fins",
    "Filters" : "Filtres",
    "Watermark" : "Filigrane",
    "Draw" : "Dessiner",
    "Resize" : "Redimensionner",
    "Image name" : "Nom de l'image",
    "Invalid image." : "Image invalide.",
    "Error while uploading the image." : "Erreur lors du téléversement de l’image.",
    "are not images" : "ne sont pas des images",
    "is not an image" : "n'est pas une image",
    "to be uploaded" : "à téléverser",
    "Crop" : "Recadrer",
    "Original" : "Original",
    "Custom" : "Personnalisé",
    "Square" : "Carré",
    "Landscape" : "Paysage",
    "Portrait" : "Portrait",
    "Ellipse" : "Ellipse",
    "Classic TV" : "Télévision classique",
    "CinemaScope" : "CinemaScope",
    "Arrow" : "Flèche",
    "Blur" : "Flou",
    "Brightness" : "Luminosité",
    "Contrast" : "Contraste",
    "Un-flip X" : "Annuler le retournement horizontal",
    "Flip X" : "Retourner horizontalement",
    "Un-flip Y" : "Annuler le retournement vertical",
    "Flip Y" : "Retourner verticalement",
    "HSV" : "TSV",
    "Hue" : "Teinte",
    "Saturation" : "Saturation",
    "Value" : "Valeur",
    "Image" : "Image",
    "Importing …" : "Importation …",
    "+ Add image" : "+ Ajouter une image",
    "Upload image" : "Téléverser l'image",
    "From gallery" : "Depuis la galerie",
    "Line" : "Ligne",
    "Pen" : "Stylo",
    "Polygon" : "Polygone",
    "Sides" : "Côtés",
    "Rectangle" : "Rectangle",
    "Corner Radius" : "Rayon des angles",
    "Width in pixels" : "Largeur en pixels",
    "Height in pixels" : "Hauteur en pixels",
    "Toggle ratio lock" : "Verrouiller/Déverrouiller les proportions",
    "Reset to original image size" : "Rétablir la taille originale de l'image",
    "Rotate" : "Faire pivoter",
    "Text" : "Texte",
    "Text spacing" : "Espacement du texte",
    "Text alignment" : "Alignement du texte",
    "Font family" : "Famille de polices",
    "Size" : "Taille",
    "Letter spacing" : "Espacement des caractères",
    "Line height" : "Hauteur de ligne",
    "Warmth" : "Température",
    "+ Add watermark" : "+ Ajouter un filigrane",
    "+ Add text watermark" : "+ Ajouter un filigrane textuel",
    "Choose watermark type" : "Choisir un type de filigrane",
    "Upload watermark" : "Téléverser un filigrane",
    "Add as text" : "Ajouter sous forme de texte",
    "Padding" : "Marge interne",
    "Paddings" : "Marges internes",
    "Shadow" : "Ombre",
    "Horizontal" : "Horizontal",
    "Vertical" : "Vertical",
    "Opacity" : "Opacité",
    "Transparency" : "Transparence",
    "Position" : "Position",
    "Stroke" : "Contour",
    "Save image as" : "Enregistrer l'image sous",
    "Extension" : "Extension",
    "Format" : "Format",
    "Name is required." : "Le nom est obligatoire.",
    "Quality" : "Qualité",
    "Saved image size (width x height)" : "Taille de l’image enregistrée (largeur × hauteur)",
    "Note that the selected crop area is lower than the applied resize which might cause quality decrease" : "La zone de recadrage sélectionnée est plus petite que la taille de redimensionnement appliquée, ce qui peut réduire la qualité.",
    "Actual size (100%)" : "Taille réelle (100 %)",
    "Fit size" : "Adapter à la taille",
    "Select image to add …" : "Sélectionner l’image à ajouter …",
    "Failed to load image." : "Échec du chargement de l'image.",
    "Menu" : "Menu",
    "Width" : "Largeur",
    "Height" : "Hauteur",
    "No preview available for this crop item" : "Aucun aperçu disponible pour cet élément de recadrage",
    "Restart" : "Recommencer",
    "Rewind {seektime}s" : "Reculer de {seektime} s",
    "Play" : "Lire",
    "Pause" : "Pause",
    "Forward {seektime}s" : "Avancer de {seektime} s",
    "Seek" : "Chercher",
    "{currentTime} of {duration}" : "{currentTime} sur {duration}",
    "Played" : "Lu",
    "Buffered" : "Mis en mémoire tampon",
    "Current time" : "Position actuelle",
    "Duration" : "Durée",
    "Volume" : "Volume",
    "Mute" : "Mettre en sourdine",
    "Unmute" : "Activer le son",
    "Enable captions" : "Activer les sous-titres",
    "Disable captions" : "Désactiver les sous-titres",
    "Enter fullscreen" : "Passer en plein écran",
    "Exit fullscreen" : "Quitter le plein écran",
    "Player for {title}" : "Lecteur de {title}",
    "Captions" : "Sous-titres",
    "Settings" : "Paramètres",
    "PIP" : "Incrustation d’image (PIP)",
    "Go back to previous menu" : "Revenir au menu précédent",
    "Speed" : "Vitesse",
    "Normal" : "Normale",
    "Loop" : "Lire en boucle",
    "Start" : "Début",
    "End" : "Fin",
    "All" : "Tout",
    "Disabled" : "Désactivé",
    "Enabled" : "Activé",
    "Ad" : "Publicité"
},
"nplurals=3; plural=(n == 0 || n == 1) ? 0 : n != 0 && n % 1000000 == 0 ? 1 : 2;");
