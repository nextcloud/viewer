...
test.describe('Viewer rename action', () => {
	...
	test('renames the current file from the viewer', async ({ page, filesApp, uploadMedia, viewer }) => {
		...
		await expect(async () => {
			expect(await viewer.currentName()).toBe('renamed.jpg')
		}).toPass()
	})
})
