...
test.describe('Viewer image editor action', () => {
	...
	test('opens the image editor and closes it on cancel', async ({ page, filesApp, viewer }) => {
		await filesApp.openFile('image1.jpg')
		await viewer.waitForOpen()

		await viewer.runAction('Edit')
	...
	test('saves and shows the edited image without a refetch', async ({ page, filesApp, viewer }) => {
		await filesApp.openFile('image1.jpg')
		await viewer.waitForOpen()
		await viewer.runAction('Edit')
	...
	test('reopens the editor from an editing=true URL', async ({ page, filesApp, viewer }) => {
		await filesApp.openFile('image1.jpg')
		await viewer.waitForOpen()
		await viewer.runAction('Edit')
		await expect(page.locator('.viewer__image-editor.image-editor')).toBeVisible()
