...
test.describe('Viewer browser history', () => {
	...
	test('reflects the open file in the URL and re-opens on refresh', async ({ page, filesApp, viewer }) => {
	...
	test('back and forward move between the shown files', async ({ page, filesApp, viewer }) => {
		await filesApp.openFilesApp()
		await filesApp.openFile('image1.jpg')
		await viewer.waitForOpen()
	...
	test('closing resets history so back does not re-open a file', async ({ page, filesApp, viewer }) => {
		await filesApp.openFilesApp()
		await filesApp.openFile('image1.jpg')
		await viewer.waitForOpen()
	...
	test('navigating out of the openfile range closes the viewer', async ({ page, filesApp, viewer }) => {
		await filesApp.openFilesApp()
		await filesApp.openFile('image1.jpg')
		await viewer.waitForOpen()
